...
//
//	deviceUniquePtr<uintType> Bin_locations(Adapt_Points, 0);
//
//	deviceUniquePtr<Particle> temp_Particles(Adapt_Points);		// this one will store the ordered particles!
//	deviceUniquePtr<floatType>		temp_values(Adapt_Points, (floatType)0);		// this one will store the values of the particles in the new order!
//
...
//		deviceUniquePtr<uintType> Bin_globalCount(CS_BBox.total_nodes(), 0);
//		deviceUniquePtr<uintType> Bin_localCount(Adapt_Points, 0);
//
...
/// 
deviceFunction
void Exh_PP_Search::operator()(const uint64_t global_id) const {
	if (global_id >= Total_Particles) { return; }

	Particle	FP_aux = Search_Particles[global_id];									// Tells me what parameter sample I'm at
	const intType	k = global_id * MaxNeighborNum;

	Index_Array[k] = global_id;
	...
	for (uintType j = i_aux * Adapt_Points; j < (i_aux + 1) * Adapt_Points; j++) {		// neighborhood where I'm searching
		...
		if (dist <= 1 && aux < MaxNeighborNum && j != global_id) {
			Index_Array[k + aux] = j;

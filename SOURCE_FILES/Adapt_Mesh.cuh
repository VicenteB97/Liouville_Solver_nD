...
__global__ void D__Wavelet_Transform__F(T* 					PDF,
										...
										const u_int32_t 	Points_at_level,
										const u_int32_t 	rescaling){

	const u_int64_t globalID = blockDim.x*blockIdx.x + threadIdx.x;
	...
	if(globalID < powf(Points_at_level, DIMENSIONS)){
		...
		u_int32_t main_node_idx = 0;

		#pragma unroll 
		for (u_int32_t j = 0; j < DIMENSIONS; j++){
		...
		for(u_int16_t d = 0; d < DIMENSIONS; d++){
			for(u_int32_t k = 0; k < powf(2,DIMENSIONS);k++){
				if( floor(positive_rem(k, pow(2, d + 1)) / pow(2, d))  == 0 ){ // this part decides whether this is an approximation node
					u_int32_t approx_coef_idx = main_node_idx;
					u_int32_t detail_coef_idx;
		...
		for(u_int32_t k = 1; k < powf(2,DIMENSIONS); k++){

			u_int32_t temp_idx = main_node_idx;
...
int ADAPT_MESH_REFINEMENT_nD(const thrust::host_vector<TYPE>& H_PDF, thrust::device_vector<TYPE>* D__PDF, std::vector<TYPE>* AdaptPDF, const gridPoint* H_Mesh, std::vector<gridPoint>* AdaptGrid, const int LvlFine, const int LvlCoarse, const int PtsPerDim) {
	...
	for (u_int16_t k = 0; k < LvlFine - LvlCoarse; k++) {

		u_int32_t Points_at_level  = PtsPerDim / rescaling;

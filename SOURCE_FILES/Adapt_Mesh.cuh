...
__global__ void D__Wavelet_Transform__F(T* 					PDF,
										...
										const UINT 	Points_at_level,
										const UINT 	rescaling){

	const uint64_t globalID = blockDim.x*blockIdx.x + threadIdx.x;

	if(globalID >= powf(Points_at_level, DIMENSIONS)) { return; }
	...
	for (uint16_t j = 0; j < DIMENSIONS; j++){
		main_node_idx += floor(positive_rem(globalID, pow(Points_at_level, j + 1)) / pow(Points_at_level, j)) * pow(PtsPerDim, j) * rescaling;
	}
	...
	for(uint16_t d = 0; d < DIMENSIONS; d++){
		for(UINT k = 0; k < powf(2,DIMENSIONS);k++){
			if( floor(positive_rem(k, pow(2, d + 1)) / pow(2, d))  == 0 ){ // this part decides whether it is an approximation node
				UINT approx_coef_idx = main_node_idx;
				UINT detail_coef_idx;
...
int16_t ADAPT_MESH_REFINEMENT_nD(const thrust::host_vector<TYPE>& H_PDF, thrust::device_vector<TYPE>* D__PDF, std::vector<TYPE>* AdaptPDF, const gridPoint* H_Mesh, std::vector<gridPoint>* AdaptGrid, const int LvlFine, const int LvlCoarse, const int PtsPerDim) {
	...
	UINT Total_Points = pow(PtsPerDim, DIMENSIONS);
	...
	for (uint16_t k = 0; k < LvlFine - LvlCoarse; k++) {

		UINT Points_at_level  = PtsPerDim / rescaling;

		uint16_t Threads = fmin(THREADS_P_BLK, pow(Points_at_level, DIMENSIONS));
		UINT Blocks  = floor((pow(Points_at_level, DIMENSIONS) - 1) / Threads) + 1;

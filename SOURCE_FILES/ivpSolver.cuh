...
class ivpSolver{
public:
	grid<PHASE_SPACE_DIMENSIONS, T> Problem_Domain;
	Distributions<T> IC_Distributions[PHASE_SPACE_DIMENSIONS];
	Distributions<T> Parameter_Distributions[PARAM_SPACE_DIMENSIONS];
	...
	int16_t buildTimeVec(){
		...
		if (error_check == -1){	std::cout << "Error building the time vector. Exiting simulation\n"; return -1;}
	...
	int16_t getDistributions(){
		...
		for (UINT p = 0; p < PARAM_SPACE_DIMENSIONS; p++){
	...
	int16_t EvolvePDF(const cudaDeviceProp* D_Properties){
		...
		std::vector<gridPoint<PHASE_SPACE_DIMENSIONS, T>>	Particle_Locations;
		std::vector<T>								Particle_Values;

		// Full array storing appended particles for all parameter samples
		std::vector<gridPoint<PHASE_SPACE_DIMENSIONS, T>>	Full_Particle_Locations;
		std::vector<T>								Full_Particle_Values;
		...
		for (UINT i = 0; i < totalSampleCount; i++) {
			Param_vec<PARAM_SPACE_DIMENSIONS, T> temp = Gather_Param_Vec<PARAM_SPACE_DIMENSIONS, T>(i, Parameter_Mesh, Samples_per_Param);
	...
	// PROBLEM DOMAIN AND INITIAL PDF
		...
		grid<PHASE_SPACE_DIMENSIONS, T> PDF_Support(IC_InfTVAL, IC_SupTVAL);
		...
		thrust::host_vector<T> PDF_at_ProbDom(Problem_Domain.Total_Nodes(), 0);	 			

		// initialize the grid and the PDF at the grid nodes (change so as to change the parameters as well)
		error_check = PDF_INITIAL_CONDITION<PHASE_SPACE_DIMENSIONS, T>(Problem_Domain, PDF_at_ProbDom, IC_Distributions); 	
		if (error_check == -1){return -1;}

		// PDF values at fixed Grid Nodes (for the GPU)
		thrust::device_vector<T> D_PDF_at_ProbDom = PDF_at_ProbDom;

		// Particle positions (for the GPU)
		thrust::device_vector<gridPoint<PHASE_SPACE_DIMENSIONS, T>> D_Particle_Locations;
		
		// PDF value at Particle positions (for the GPU)
		thrust::device_vector<T> D_Particle_Values;
	...
	//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
		...
		// Counting the number of nodes that will be advected by the ODE integrator, and num. of neighbors for the RBF interpolation
		UINT AMR_ActiveNodeCount, MaxNeighborNum;
		...
		const T 	ConjGrad_Tolerance = TOLERANCE_ConjGrad, 
					RBF_SupportRadius  = DISC_RADIUS * Problem_Domain.Discr_length();

		// Now we make a slightly larger domain for the computations:
		grid<PHASE_SPACE_DIMENSIONS, T> Expanded_Domain;
	...
	// START OF SIMULATION STEPS AS SUCH
		...
		thrust::copy(PDF_at_ProbDom.begin(), PDF_at_ProbDom.end(), &storeFrames[0]);
		...
		while (simStepCount < time_vector.size() - 1 && error_check != -1) {
			...
			auto startTimeSeconds = std::chrono::high_resolution_clock::now();
				...
				error_check = setInitialParticles<PHASE_SPACE_DIMENSIONS, T>(PDF_at_ProbDom, D_PDF_at_ProbDom,
			...
			AMR_ActiveNodeCount = Particle_Locations.size();
			...
			MaxNeighborNum = round(fmin(pow(2 * round(DISC_RADIUS) + 1, PHASE_SPACE_DIMENSIONS), AMR_ActiveNodeCount));
			...
			thrust::device_vector<T>	D_Mat_Vals(MaxNeighborNum * AMR_ActiveNodeCount, 0);
			...
			startTimeSeconds = std::chrono::high_resolution_clock::now();
				...
				if (AMR_ActiveNodeCount < ptSEARCH_THRESHOLD) {
					Exh_PP_Search<PHASE_SPACE_DIMENSIONS,T> << <Blocks, Threads >> > (rpc(D_Particle_Locations, 0),
					...
					error_check = _CS_Neighbor_Search<PHASE_SPACE_DIMENSIONS, T>(D_Particle_Locations,
																	D_Particle_Values,
																	D_Mat_Indx,
																	D_Mat_Vals,
																	D_Mat_NumNeighbors,
			...
			thrust::device_vector<T> D_lambdas(AMR_ActiveNodeCount, 0);
			...
			if (time_vector[simStepCount].impulse) {
				...
				error_check = IMPULSE_TRANSFORM_PDF<PHASE_SPACE_DIMENSIONS, T>(D_PDF_at_ProbDom,
																		Particle_Locations,
																		Particle_Values,
				...
				PDF_at_ProbDom = D_PDF_at_ProbDom;

				// Store info in cumulative variable
				thrust::copy(PDF_at_ProbDom.begin(), PDF_at_ProbDom.end(), &(*storeFrames)[simStepCount * Problem_Domain.Total_Nodes()]);
			...
			// 1.2.- COMPUTE THE SMOOTH EVOLUTION VIA LIOUVILLE GIBBS/CONTINUITY EQUATION
				...
				const UINT Bytes_per_sample = (UINT)AMR_ActiveNodeCount * (sizeof(T) * 2 + sizeof(gridPoint<PHASE_SPACE_DIMENSIONS, T>));

				// Set number of random samples to work with at the same time
				UINT Samples_PerBlk = (UINT)fmin((UINT)totalSampleCount, MAX_BYTES_USEABLE / Bytes_per_sample);
				...
				for (UINT b = 0; b < total_simulation_blocks; b++) {
					...
					startTimeSeconds = std::chrono::high_resolution_clock::now();
						ODE_INTEGRATE<PHASE_SPACE_DIMENSIONS, PARAM_SPACE_DIMENSIONS, T><< <Blocks, Threads >> > (rpc(D_Particle_Locations, 0),
					...
					D_PDF_at_ProbDom.resize(Problem_Domain.Total_Nodes(), 0);	// PDF is reset to 0, so that we may use atomic adding at the remeshing step
					
					Threads = fminf(THREADS_P_BLK, ActiveNodes_PerBlk);
					Blocks  = floorf((ActiveNodes_PerBlk - 1) / Threads) + 1;

					startTimeSeconds = std::chrono::high_resolution_clock::now();
					RESTART_GRID_FIND_GN<PHASE_SPACE_DIMENSIONS, PARAM_SPACE_DIMENSIONS, T> << < Blocks, Threads >> > (	rpc(D_Particle_Locations, 0),
				...
				CORRECTION<T> << <Blocks, Threads >> > (rpc(D_PDF_at_ProbDom, 0), Problem_Domain.Total_Nodes());
				gpuError_Check(cudaDeviceSynchronize());

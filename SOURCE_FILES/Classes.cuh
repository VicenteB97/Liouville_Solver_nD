...
#define ptSEARCH_THRESHOLD 500000 // The min. number of particles per sample where we use the counting sort search (not worth it yet)
...
public:
	...
	Logger(const uint32_t& size) {
		subFrame_time.resize(4 * size, (double)0);
	}

	// Get the total frame time:
	inline double FrameTime(const uint32_t& timeStep) const {
		return (subFrame_time[4 * timeStep] + subFrame_time[4 * timeStep + 1] + subFrame_time[4 * timeStep + 2] + subFrame_time[4 * timeStep + 3]);
	}
	...
	void writeToCLI(const uint16_t& Log_Lvl, const uint32_t& time_step) const {
		...
		switch(Log_Lvl){
			...
			case 2:
				...
				std::cout << "AMR timing: " << subFrame_time[4 * time_step] << "s.\n";
				std::cout << "Advection timing: " << subFrame_time[4 * time_step + 1] << "s.\n";
				std::cout << "Interpolation timing: " << subFrame_time[4 * time_step + 2] << "s.\n";
				std::cout << "Reinitialization timing: " << subFrame_time[4 * time_step + 3] << "s.\n";
				std::cout << "Total frame time: " << this->FrameTime(time_step) << "s.\n";
				break;

			default:
				std::cout << "Error in the log-level choice. No output will be generated.\n";
				break;
		}
	}
	...
	void writeToFile() const {
		...
		Log_fileName.append("LogFile.csv");

		std::ofstream log_file(Log_fileName, std::ios::out);
		assert(log_file.is_open());

		// Create each line to append
		for (uint16_t k = 0; k < 5; k++) {
			std::string temp = "AMR, Advection, Interpolation, Reinitialization, Total\n";

			log_file << temp;

			for (uint32_t i = 0; i < subFrame_time.size(); i++) {

				temp = std::to_string(subFrame_time[	 4 * i + 0]);
				temp.append(std::to_string(subFrame_time[4 * i + 1]));
				temp.append(std::to_string(subFrame_time[4 * i + 2]));
				temp.append(std::to_string(subFrame_time[4 * i + 3]));

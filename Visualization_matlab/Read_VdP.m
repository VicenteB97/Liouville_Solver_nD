...
Info=readcell('../output/prod_VDP_r_9_reinit_5_Simulation_info_0.csv');
fileID = fopen('../output/prod_VDP_r_9_reinit_5_Mean_PDFs_0.bin');

output_name = ['../output/liouville_VDP_r_9_reinit_5_time_4.pdf'];
output_name_jpdf = ['../output/liouville_VDP_jpdf_r_9_reinit_5_time_4.pdf'];
output_name_sol = ['../output/liouville_VDP_r_9_reinit_5_time_4.pdf'];
...
for k=FTIME:FTIME
    ...
    figure(10)
    plot(X,MargX(:,k))
    title('L: Van der Pol per-component PDFs at $t = t_{f}$','FontSize', 16);
    xlim([-4 4])
    ylim([0 5])
    xticks(-4:4)
    yticks(0:5)
    ...
    DATA1=F_Output(:,:,k)';
    DATA1(DATA1 == 0) = NaN;
    surf(X,Y,DATA1,EdgeColor = 'none');
    view(0,90);
    grid off;
    colormap(flipud(hot))
    title('L: Van der Pol JPDF at $t = t_{f}$','FontSize', 16);
    colorbar;

...
output_name = ['../output/prod_VDP_r_9_reinit_5_time_2.5.pdf'];
output_name_jpdf = ['../output/prod_VDP_jpdf_r_9_reinit_5_time_2.5.pdf'];
output_name_sol = ['../output/prod_VDP_r_9_reinit_5_time_2.5.pdf'];
...
for k=FTIME-1:FTIME
    ...
    figure(10)
    plot(X,MargX(:,k))
    title('Van der Pol per-component PDFs at $t = 2.5$ (Liouville)');
    xlim([-4 4])
    ylim([0 5])
    xticks(-4:4)
    yticks(0:5)
    ...
    if Show_AMR
        ...
        grid off; 
        colormap('parula')
        title('Van der Pol JPDF at $t = 2.5$ (Liouville)'); 
        colorbar;
